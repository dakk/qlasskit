    ...
    def zx_simplify(self):
        ...
        for g, w, p in self.gates:
            if (
                ...
                or (isinstance(g, gates.MCtrl)
                and isinstance(g.gate, gates.X))
            ):
                if len(w) == 3:
                    zc.add_gate('TOF', *w)
                elif len(w) == 2:
        ...
        for g in zc2.gates:
            ...
            if hasattr(g, "control"): w.append(g.control)
            if hasattr(g, "ctrl1"): w.append(g.ctrl1)
            if hasattr(g, "ctrl2"): w.append(g.ctrl2)
            if hasattr(g, "target"): w.append(g.target)
